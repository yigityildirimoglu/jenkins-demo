...
PY
    ...
    // *** GÜNCELLENEN AŞAMA BURASI ***
    stage('Deploy Blue/Green') {
      steps {
        // 1. AWS kimlik bilgilerini yükle
        withCredentials([[$class: 'AmazonWebServicesCredentialsBinding', credentialsId: 'aws-deploy-credentials']]) {
          
          // 2. SSH kimlik bilgilerini 'sshagent' ile yükle
          // Artık 'deploy-server-ssh-key' kimlik bilgisi kullanılıyor
          sshagent(credentials: ['deploy-server-ssh-key']) {
            
            // 3. Tüm deploy script'i bu iki wrapper'ın (sarmalayıcının) içinde
            script {
              ...
              // Bu SSH komutu artık 'sshagent' tarafından sağlanan anahtarı kullanacak
              // Diskteki '/var/lib/jenkins/.ssh/id_rsa' dosyasına bağımlı DEĞİL.
              sh(
                script: """
